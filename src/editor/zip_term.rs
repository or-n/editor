...
impl Zip {
    ...
    pub fn down2(mut self) -> Result<Self, Self> {
        match *self.term {
            T::Id(_) => Err(self),
            T::Abstract(id, cont) => {
                self.went.push(Go::AbstractContinue(id));
                self.term = cont;
                Ok(self)
            }
            T::Let(value, abstraction) => {
                self.went.push(Go::LetAbstract(value));
                self.term = abstraction;
                Ok(self)
            }
            T::I(_) => Err(self),
            T::Pair(a, b) => {
                self.went.push(Go::PairB(a));
                self.term = b;
                Ok(self)
            }
            T::If(mut branches) => {
                if let Some(branch) = branches.pop() {
                    self.went.push(Go::IfBranch(branches, vec![]));
                    self.term = Box::new(branch);
                    Ok(self)
                } else {
                    self.term = Box::new(T::If(branches));
                    Err(self)
                }
            }
        }
    ...
    pub fn up2(mut self) -> Result<Self, Self> {
        if let Some(go) = self.went.pop() {
            match go {
                Go::AbstractContinue(id) => {
                    self.term = Box::new(T::Abstract(id, self.term));
                }
                Go::LetValue(abstraction) => {
                    self.term = Box::new(T::Let(self.term, abstraction));
                }
                Go::LetAbstract(value) => {
                    self.term = Box::new(T::Let(value, self.term));
                }
                Go::PairA(b) => {
                    self.term = Box::new(T::Let(self.term, b));
                }
                Go::PairB(a) => {
                    self.term = Box::new(T::Let(a, self.term));
                }
                Go::IfBranch(mut before, rev_after) => {
                    before.push(*self.term);
                    before.extend(rev_after.into_iter().rev());
                    self.term = Box::new(T::If(before));
                }
            }
    ...
    pub fn left2(mut self) -> Result<Self, Self> {
        if let Some(go) = self.went.pop() {
            match go {
                Go::AbstractContinue(_) => Err(self),
                Go::LetValue(_) => Err(self),
                Go::LetAbstract(value) => {
                    self.went.push(Go::LetValue(self.term));
                    self.term = value;
                    Ok(self)
                }
                Go::PairA(_) => Err(self),
                Go::PairB(a) => {
                    self.went.push(Go::PairA(self.term));
                    self.term = a;
                    Ok(self)
                }
                Go::IfBranch(mut before, mut rev_after) => {
                    if let Some(next) = before.pop() {
                        rev_after.push(*self.term);
                        self.term = Box::new(next);
                        Ok(self)
                    } else {
                        Err(self)
    ...
    pub fn right2(mut self) -> Result<Self, Self> {
        if let Some(go) = self.went.pop() {
            match go {
                Go::AbstractContinue(_) => Err(self),
                Go::LetValue(abstraction) => {
                    self.went.push(Go::LetAbstract(self.term));
                    self.term = abstraction;
                    Ok(self)
                }
                Go::LetAbstract(_) => Err(self),
                Go::PairA(b) => {
                    self.went.push(Go::PairB(self.term));
                    self.term = b;
                    Ok(self)
                }
                Go::PairB(_) => Err(self),
